...
function SearchContent() {
  ...
  const startAnalysis = async (barcode: string) => {
    ...
    try {
      ...
      // Enforce minimum 3-second delay for backend calculations feeling
              ...
              >
                ...
                <div
                  onClick={() => {
                    ...
                    if (expandedCard !== product.barcode) {
                      setAnalysisResult(null)
                      setAnalysisError("")
                    }
                  }}
                      ...
                      <div className="p-6 space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-border/60">
                          ...
                          {isAnalyzing && (
                            <button
                              disabled
                              className="w-full sm:w-auto px-6 py-2.5 rounded-full bg-gradient-to-r from-purple-500 via-pink-500 to-teal-500 animate-fluid-flow text-white font-bold text-sm glow-effect shadow-md flex items-center justify-center gap-2"
                            >
                        ...
                        {/* Analysis Errors */}
                        {analysisError && (
                          <div className="p-3 bg-red-500/10 border border-red-500/30 text-red-500 text-sm rounded-xl flex items-center gap-2">
                        ...
                        {analysisResult && (
                          ...
                          >
                            ...
                            {(() => {
                              ...
                              return (
                                <div className={`p-5 rounded-2xl border ${risk.bg} ring-4 ${risk.ring}`}>
                                  <h4 className="text-lg font-bold font-syne mb-1 text-foreground">Analysis Complete</h4>
                                  <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border ${risk.bg} ${risk.color} font-bold text-sm mt-2`}>
                                    {risk.icon}
                            ...
                            {Object.keys(analysisResult.analysis.final_scores || {}).length > 0 && (
                              <div className="rounded-2xl border border-border bg-background overflow-hidden">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    setExpandedSection(expandedSection === "conditions" ? null : "conditions")
                                  }}
                                  className="w-full p-4 flex items-center justify-between hover:bg-muted/20 transition-colors"
                                >
                                  <span className="font-bold text-sm font-syne flex items-center gap-2">
                                    <Zap className="w-4.5 h-4.5 text-yellow-400" /> Condition-Specific Risks
                                  </span>
                            ...
                            <div className="rounded-2xl border border-border bg-background overflow-hidden">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
                                  setExpandedSection(expandedSection === "additives" ? null : "additives")
                                }}
                                className="w-full p-4 flex items-center justify-between hover:bg-muted/20 transition-colors"
                              >
                                <span className="font-bold text-sm font-syne flex items-center gap-2">
                                        ...
                                        <div className="mt-4 pt-4 border-t border-border/60">
                                          ...
                                          <div className="flex flex-wrap gap-1.5">
                                            {analysisResult.analysis.safe_additives.map((sa, idx) => (
                                              <span 
                                                key={idx} 
                                                className="text-[10px] bg-muted/40 text-muted-foreground px-2.5 py-1 rounded-full border border-border/40 font-medium"
                                              >
